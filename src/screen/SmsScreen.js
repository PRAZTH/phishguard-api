...
export default function SmsScreen({ onNavigate }) {
  ...
  const openMessagesApp = async () => {
    ...
    try { await Linking.openURL(url); } catch (err) { Alert.alert("Error", "Could not open system SMS utility."); }
  };

  const handleScan = async () => {
    if (!smsText) { Alert.alert("Empty Input", "Please provide textual data containing a link first."); return; }
    
    let foundUrl = extractUrlFromText(smsText);
    if (!foundUrl) { 
        Alert.alert("Link M.I.A", "No clean structured hyperlink domain parsed from input."); 
        return; 
    }
    ...
    try {
      ...
      if (data.error) {
        Alert.alert("Process Exception", data.error);
      } else {
        // Physical haptic device alarms 
        if (data.result !== "Safe") {
        ...
        Alert.alert(
            data.result === "Safe" ? "✅ UNCOMPROMISED LINK" : "⚠️ PHISHING RISK DETECTED",
            `System Verdict: ${data.result}\n\n${data.explanation ? data.explanation.join('\n') : ''}`
        );
      }
    } catch (error) {
      setLoading(false);
      Alert.alert("Network Disturbance", "Backend API node unreachable. Confirm Render initialization cycles.");
    }
  };

  return (
    <View style={styles.mainContainer}>
      <ImageBackground source={require('../../assets/smsbg.jpeg')} style={styles.backgroundImage} resizeMode="cover">
        ...
        <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={{ flex: 1 }}>
          <ScrollView contentContainerStyle={styles.scrollContainer}>
            <View style={styles.header}>
                ...
                <Text style={styles.subtitle}>Audit text vectors seamlessly</Text>
            </View>
            
            <View style={styles.card}>
                <View style={styles.instructionBox}>
                    <Text style={styles.stepText}>1. Copy SMS container text details.</Text>
                    <Text style={styles.stepText}>2. Return to this module view.</Text>
                    <Text style={styles.stepText}>3. Auto-hook populates terminal context.</Text>
                </View>
                ...
                <TouchableOpacity style={styles.openAppButton} onPress={openMessagesApp}>
                    ...
                    <Text style={styles.btnTextSmall}>LAUNCH SMS APP</Text>
                </TouchableOpacity>

                <TextInput 
                    style={styles.textArea} 
                    placeholder="Waiting for device text clipboard data..." 
                    value={smsText} 
                ...
                <TouchableOpacity style={styles.orangeButton} onPress={handleScan} disabled={loading}>
                    {loading ? <ActivityIndicator color="#fff" /> : <Text style={styles.btnText}>🔍 INITIATE SCANNERS</Text>}
                </TouchableOpacity>
                ...
                <View style={styles.buttonRow}>
                    <TouchableOpacity style={styles.greyButton} onPress={async () => {
                        ...
                        <Text style={styles.btnTextSmall}> Force Paste</Text>
                    </TouchableOpacity>
                    <TouchableOpacity style={styles.greyButton} onPress={() => setSmsText('')}>
                        <Ionicons name="trash-outline" size={18} color="#fff" />
                        <Text style={styles.btnTextSmall}> Flush</Text>
                    </TouchableOpacity>
                </View>
